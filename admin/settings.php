 ...
 */
function kwik_ai_tags_get_api_endpoint()
{
  $provider = get_option('kwik_ai_ai_provider', 'ollama');
...
/**
 * Get API headers for OpenRouter/OpenAI requests
 *
 * @param string $provider
 * @return array
 */
function kwik_ai_tags_get_api_headers($provider)
{
  $headers = array(
    'Content-Type' => 'application/json',
  );
 ...
 * @return string
 */
function kwik_ai_tags_get_api_key($provider)
{
  if ($provider === 'openrouter') {
