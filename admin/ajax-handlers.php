...
function kwik_ai_tags_ajax_generate()
{
  ...
  if (!current_user_can('edit_posts')) {
    ...
    wp_die(esc_html__('You do not have sufficient permissions.', 'kwik-ai'));
  }

  // Rate limiting: 1 request per 15 seconds per user
  if (!kwik_ai_check_rate_limit('kwik_ai_tags_generate', 15)) {
    wp_send_json_error(__('Please wait a moment before generating tags again.', 'kwik-ai'));
  }

  if (!isset($_POST['post_id'])) {
    wp_send_json_error(__('Missing post ID.', 'kwik-ai'));
  }
  ...
  if (defined('WP_DEBUG') && WP_DEBUG) {
    kwik_ai_log('Kwik AI: Generated tags: ' . wp_json_encode($tags));
  }
  ...
  if ($tags === false) {
    ...
    wp_send_json_error(__('Failed to generate tags. Please check your AI provider connection and try again.', 'kwik-ai'));
  }
  ...
  if (empty($tags)) {
    ...
    wp_send_json_error(__('No tags were generated. Try adding more descriptive images.', 'kwik-ai'));
  }
...
function kwik_ai_tags_ajax_apply()
{
  ...
  if (!isset($_POST['post_id']) || !isset($_POST['tags'])) {
    wp_send_json_error(__('Missing required fields.', 'kwik-ai'));
  }
  ...
  if (empty($tag_array)) {
    wp_send_json_error(__('No valid tags to apply.', 'kwik-ai'));
  }
  ...
  /* translators: %d: number of tags applied */
  wp_send_json_success(sprintf(__('Successfully applied %d tags to the post!', 'kwik-ai'), count($tag_array)));
}
...
function kwik_ai_description_ajax_generate()
{
  ...
  if (!kwik_ai_check_rate_limit('kwik_ai_description_generate', 15)) {
    wp_send_json_error(__('Please wait a moment before generating descriptions again.', 'kwik-ai'));
  }
  ...
  if ($excerpt === false) {
    ...
    wp_send_json_error(__('Failed to generate excerpt. Please check your AI provider connection and try again.', 'kwik-ai'));
  }

  if (empty($excerpt)) {
    kwik_ai_log('Kwik AI: No excerpt generated');
    wp_send_json_error(__('No excerpt was generated. Try adding more content to the post first.', 'kwik-ai'));
  }
...
function kwik_ai_description_ajax_apply()
{
  ...
  if (!$description) {
    wp_send_json_error(__('No description provided.', 'kwik-ai'));
  }

  // Description will be inserted into the editor by JavaScript
  // No need to modify post_content here to avoid duplication

  wp_send_json_success(__('Description ready to be inserted into the editor!', 'kwik-ai'));
}
...
function kwik_ai_tags_ajax_fetch_models()
{
  ...
  if (!current_user_can('manage_options')) {
    wp_send_json_error(__('You do not have sufficient permissions.', 'kwik-ai'));
  }
  ...
  if (!is_array($models) || empty($models)) {
    wp_send_json_error(__('Failed to fetch models. Please check your endpoint URL and API key.', 'kwik-ai'));
  }
...
function kwik_ai_tags_ajax_test_connection()
{
  ...
  if (($provider === 'openrouter' || $provider === 'openai') && empty($api_key)) {
    wp_send_json_error(__('An API key is required for this provider.', 'kwik-ai'));
  }

  $models = kwik_ai_tags_fetch_models_live($provider, $endpoint, $api_key);

  if (!is_array($models) || empty($models)) {
    wp_send_json_error(__('Could not connect. Please check your endpoint URL and API key.', 'kwik-ai'));
  }
  ...
  $message = sprintf(
    ...
    _n('Connected — %d model available.', 'Connected — %d models available.', $count, 'kwik-ai'),
    $count
  );
  ...
  if ($model !== '' && !$model_found) {
    $message .= ' ' . sprintf(
      ...
      __('Note: the selected model "%s" was not found in the list.', 'kwik-ai'),
      $model
    );
  }
...
function kwik_ai_fal_ajax_refresh_models()
{
  ...
  if (empty($models)) {
    wp_send_json_error(__('Could not reach FAL.AI. Please try again.', 'kwik-ai'));
  }
...
function kwik_ai_featured_image_ajax_prompt()
{
  ...
  if (!kwik_ai_check_rate_limit('kwik_ai_featured_image_prompt', 10)) {
    wp_send_json_error(__('Please wait a moment before generating another prompt.', 'kwik-ai'));
  }
  ...
  if (empty($result['prompt'])) {
    wp_send_json_error(__('Could not build a prompt. Add a title or content to the post.', 'kwik-ai'));
  }
...
function kwik_ai_featured_image_ajax_generate()
{
  ...
  if (!kwik_ai_check_rate_limit('kwik_ai_featured_image_generate', 15)) {
    wp_send_json_error(__('Please wait a moment before generating another image.', 'kwik-ai'));
  }
  ...
  if (trim($prompt) === '') {
    wp_send_json_error(__('Please generate or enter a prompt before creating an image.', 'kwik-ai'));
  }
...
function kwik_ai_featured_image_ajax_status()
{
  ...
  if (empty($_POST['request_id'])) {
    wp_send_json_error(__('Missing request ID.', 'kwik-ai'));
  }
  ...
  if (!is_array($job) || (int) $job['user_id'] !== get_current_user_id()) {
    wp_send_json_error(__('This image request could not be found. Please generate again.', 'kwik-ai'));
  }
...
function kwik_ai_featured_image_ajax_apply()
{
  ...
  if (empty($job['image_url'])) {
    wp_send_json_error(__('The image is not ready yet. Please wait for generation to finish.', 'kwik-ai'));
  }
  ...
  wp_send_json_success(array(
    ...
    'message'       => __('Featured image set!', 'kwik-ai'),
  ));
}
