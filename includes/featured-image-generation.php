...
function kwik_ai_featured_image_submit(int $post_id, string $prompt = '')
{
  ...
  if (trim($prompt) === '') {
    return new WP_Error('kwik_ai_fal_no_prompt', __('Could not build an image prompt. Add a title or content to the post, or enter a prompt.', KWIK_AI_DOMAIN));
  }
...
function kwik_ai_featured_image_set_as_thumbnail(int $post_id, string $image_url)
{
  ...
  if (!is_string($host) || !preg_match('/(^|\.)fal\.media$/', $host)) {
    return new WP_Error('kwik_ai_fal_bad_image_host', __('Refusing to download an image from an unexpected host.', KWIK_AI_DOMAIN));
  }
  ...
  $description = sprintf(
    ...
    __('AI-generated featured image for "%s"', KWIK_AI_DOMAIN),
