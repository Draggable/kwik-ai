...
function kwik_ai_fal_submit_request(string $prompt, array $args = array())
{
  ...
  if ($api_key === '') {
    return new WP_Error('kwik_ai_fal_no_key', __('No FAL.AI API key is configured. Add one in Settings > KWIK AI.', KWIK_AI_DOMAIN));
  }

  $prompt = trim($prompt);
  if ($prompt === '') {
    return new WP_Error('kwik_ai_fal_no_prompt', __('No prompt was generated for the image.', KWIK_AI_DOMAIN));
  }
  ...
  if (!is_array($data) || empty($data['request_id'])) {
    return new WP_Error('kwik_ai_fal_bad_response', __('FAL.AI returned an unexpected response when submitting the job.', KWIK_AI_DOMAIN));
  }
...
function kwik_ai_fal_check_status(string $status_url)
{
  ...
  $api_key = kwik_ai_fal_get_api_key();
  if ($api_key === '') {
    return new WP_Error('kwik_ai_fal_no_key', __('No FAL.AI API key is configured.', KWIK_AI_DOMAIN));
  }
  ...
  if (!is_array($data) || empty($data['status'])) {
    return new WP_Error('kwik_ai_fal_bad_response', __('FAL.AI returned an unexpected status response.', KWIK_AI_DOMAIN));
  }
...
function kwik_ai_fal_get_result(string $response_url)
{
  ...
  if (!is_array($data) || empty($data['images'][0]['url'])) {
    return new WP_Error('kwik_ai_fal_no_image', __('FAL.AI did not return an image. Try regenerating.', KWIK_AI_DOMAIN));
  }
...
function kwik_ai_fal_validate_queue_url(string $url)
{
  ...
  if ($host !== KWIK_AI_FAL_QUEUE_HOST) {
    return new WP_Error('kwik_ai_fal_bad_url', __('Refusing to contact an unexpected FAL.AI URL.', KWIK_AI_DOMAIN));
  }
  return true;
}
...
function kwik_ai_fal_extract_error_message(string $raw, int $code): string
{
  ...
  if ($code === 401 || $code === 403) {
    return __('FAL.AI rejected the API key. Check your key in Settings > KWIK AI.', KWIK_AI_DOMAIN);
  }

  /* translators: %d: HTTP status code */
  return sprintf(__('FAL.AI request failed (HTTP %d).', KWIK_AI_DOMAIN), $code);
}
...
function kwik_ai_fal_test_connection()
{
  if (!kwik_ai_fal_has_api_key()) {
    return __('No FAL.AI API key configured.', KWIK_AI_DOMAIN);
  }
